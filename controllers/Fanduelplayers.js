    ...
    request(urlNFL, function(error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            for (var i = 0; i < body.feed.entry.length; i++) {
                ...
                var turtle = [myNFL.gsx$player.$t, myNFL.gsx$sal.$t,
                  myNFL.gsx$injurystatus.$t, myNFL.gsx$pos.$t, myNFL.gsx$team.$t,
                  myNFL.gsx$opp.$t, myNFL.gsx$rvp.$t, myNFL.gsx$totalopp.$t, myNFL.gsx$teamvaluevspos.$t, myNFL.gsx$winvsopp.$t, myNFL.gsx$ou.$t, myNFL.gsx$linechange.$t, myNFL.gsx$salchange.$t, myNFL.gsx$criteriascore.$t, myNFL.gsx$critrank.$t, myNFL.gsx$ppa.$t, myNFL.gsx$snapcount.$t, myNFL.gsx$fps.$t, myNFL.gsx$recpre.$t]
                fanduel.createfandueldataNFL(['Player', 'Sal', "Injury_Status","POS", 'Team', 'Opp',"RvP","TotalOpp","TeamValueVsPos","WinVsOpp","OU","Line_Change","Sal_Change","CriteriaScore","CritRank","PPA","SnapCount","FPS", "RecPre"
        ...
        if (!error && response.statusCode == 200) {
            ...
            for (var i = 0; i < body.feed.entry.length; i++) {
                ...
                fanduel.createfandueldata(['Player', "HomeAway", "Position", "Team", "Opp", "Mycondition", "details", "posid", "AvgMinsL10", "ProjMins", "Salary", "Useage", "PPGL10", "PPM", "OVP", "GLAdjust", "TotalAdjust", "ProjScore", "GameLine", "ProjectedPts", "Value", "Ownership", "PathCompare", "ExpectedValue", "NetValue",
                    ...
                    turtle[29], turtle[30], turtle[31], turtle[31]
                ], function(nbadata) {
